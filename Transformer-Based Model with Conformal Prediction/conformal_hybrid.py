...
def load_and_preprocess_data_hybrid():
    ...
    for col in feature_order:
        ...
        # After filling NaN values for "release_date"
    for df in [train_df, test_df]:
        # Convert to datetime, coercing errors (so empty strings become NaT)
        df["release_date"] = pd.to_datetime(
            df["release_date"], format="%Y-%m-%d", errors="coerce"
        )

    # Find the oldest date across both train and test sets (ignoring NaT values)
    min_date = pd.concat(
        [train_df["release_date"], test_df["release_date"]]
    ).min()

    # Convert release_date to the number of days since min_date,
    # and fill missing dates with 0 (or another appropriate value)
    for df in [train_df, test_df]:
        df["release_date"] = (
            (df["release_date"] - min_date).dt.days.fillna(0).astype(int)
        )
    ...
    for train_index, val_index in kf.split(train_df):
        ...
        train_numeric = train_df[numerical_features].values[train_index]
        val_numeric = train_df[numerical_features].values[val_index]
        ...
        data.append(
            {
                ...
                "ensemble_test_numeric": ensemble_test_df[
                    numerical_features
                ].values,
...
class MovieDatasetHybrid(Dataset):
    def __init__(
        self,
        texts,
        numerical_features,
        ...
        if self.labels is not None:
            return {
                ...
                "numerical_features": torch.tensor(
                    self.numerical_features[idx], dtype=torch.float
                ),
...
def train_model_hybrid(
    ...
    log_steps=10,
    name="model",
    save_dir="models_hybrid/",
):
    train_dataset = MovieDatasetHybrid(
        texts=data["train_texts"],
        numerical_features=data["train_numeric"],
        labels=data["train_labels"],
        tokenizer=tokenizer,
    )

    val_dataset = MovieDatasetHybrid(
        texts=data["val_texts"],
        numerical_features=data["val_numeric"],
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(
            ...
            outputs = model(
                input_ids=input_ids,
                input_numerical=numerical_features,
    ...
    # plt.tight_layout()
    plt.suptitle(
        f"Training Metrics for {name}, "
        f"lr={learning_rate}, "
        f"hidden dim 1={model.linear_hidden_dimension_1}, "
        f"hidden dim 2={model.linear_hidden_dimension_2}, "
        f"d={model.dropout_rate}, "
        f"wd={weight_decay}",
    )
    plt.savefig(
        Path(save_dir, f"{name}_training_metrics_detailed.pdf"), dpi=300
    )
    plt.show()
    ...
    metrics_table.to_csv(
        Path(save_dir, f"{name}_training_metrics_per_epoch.csv"), index=False
    )
    step_metrics.to_csv(
        Path(save_dir, f"{name}_training_metrics_per_step.csv"), index=False
    )
...
def compute_calibration_scores_hybrid_max(
    model, data, tokenizer, batch_size=16
):
    model.eval()

    cal_dataset = MovieDatasetHybrid(
        texts=data["val_texts"],
        numerical_features=data["val_numeric"],
        labels=data["val_labels"],
        tokenizer=tokenizer,
...
# ===== Functions for conformal prediction =====
def compute_calibration_scores_hybrid(model, data, tokenizer, batch_size=16):
    model.eval()
...
def predict_with_conformal_hybrid_max(
    model,
    data,
    tokenizer,
    nonconformity_scores,
    ...
    test_dataset = MovieDatasetHybrid(
        texts=data[test_field],
        numerical_features=data["test_numeric"],
        tokenizer=tokenizer,
    )
...
# ===== BASIC Functions for conformal prediction for submission =====
def predict_with_conformal_hybrid(
    model,
    data,
    tokenizer,
    nonconformity_scores,
...
def evaluate_conformal_prediction_hybrid(
    model,
    data,
    tokenizer,
    # nonconformity_scores,
    ...
    val_texts = data["val_texts"][val_idx]
    val_numeric = data["val_numeric"][val_idx]
    val_labels = data["val_labels"][val_idx]
    eval_texts = data["val_texts"][eval_idx]
    eval_numeric = data["val_numeric"][eval_idx]
    eval_labels = data["val_labels"][eval_idx]
    ...
    cal_data = {
        ...
        "ensemble_test_numeric": data["ensemble_test_numeric"],
...
def find_optimal_alpha_hybrid(
    ...
    for alpha in alpha_values:
        print(f"\nTesting alpha = {alpha}:")
        metric, coverage, length, q = evaluate_conformal_prediction_hybrid(
            model,
            data,
            tokenizer,
            # nonconformity_scores,
    ...
    return best_alpha, best_q, best_metric
